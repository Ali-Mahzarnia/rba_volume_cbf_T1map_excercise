...
for (i in 1:length(file_list)) {
  ...
  #if (sum(temp$volume_mm3[2:len])==0) {volumes[i,2:len] = 0}
  #else 
  ...
  temp[missings,]$volume_mm3 =0 
  temp[missings,]$structure = Atlas$Structure[Atlas$index%in%missings] 
  
  
  
    volumes[i,2:333]=temp$volume_mm3[1:332]/ sum(temp$volume_mm3[1:332])
